...
export default function Nav(props: NavProps) {
  ...
  const userWidgetRef = useRef<any>(null);
  useOnClickOutside(userWidgetRef, () => setAvatarDropdownOpen(false));
          ...
          <AnimatePresence>
            {avatarDropdownOpen && (
              <motion.div
                ...
                className="absolute right-0 mt-2 mr-4 bg-white rounded-lg shadow-lg z-50"
              >
                <div className="flex flex-col">
                  <p className="px-4 pt-3 pb-2 rounded-t-lg text-xs">Signed in as {firstName}</p>
                  <p className="nav-dropdown-option pt-2 pb-2" onClick={() => goTo("trends")}>
                    Trends
                  </p>
                  <p className="nav-dropdown-option pt-2 pb-2" onClick={() => goTo("journal")}>
                    Journal
                  </p>
                  <p className="nav-dropdown-option pt-2 pb-2" onClick={() => goTo("pantry")}>
                    Pantry
                  </p>
                  <p className="nav-dropdown-option pt-2 pb-2" onClick={() => goTo("profile")}>
                    Profile
                  </p>
                  <p className="nav-dropdown-option pb-3 pt-2 rounded-b-lg" onClick={doSignOut}>
                    Log out
                  </p>
                </div>
              </motion.div>
